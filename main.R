...
cities_counties <- list(new_york = c("New York", "Kings", "Bronx", "Richmond", "Queens"),
                        ...
                        memphis = c("Shelby"),
                        baltimore = c("Baltimore"), # city of baltimore does not belong to any county 
...
for (i in 20:length(unique(cities_states))) {
...
sf::read_sf(
  paste0('CleanedData/CityCouncilDistrict/',
         str_replace_all(tolower(cities[36]), " ", "_"), '_city_council_district.shp')) %>% glimpse()
