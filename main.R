...
# Create block- and council district- level intersenction
for (i in 7:11) {
    ...
    intersection_weighted_block <- block_precinct_council_district_intersection %>%
      ...
      dplyr::select(GEOID, all_of(demographic_columns), 
                    ...
                    all_of(council_district_id), area) %>% # Select relevant columns only 
    ...
    intersection_weighted_council_district <- block_precinct_council_district_intersection %>%
      as_tibble() %>%
      dplyr::select(GEOID, all_of(demographic_columns), 
                    all_of(precinct_id), all_of(partisan_columns), 
                    all_of(council_district_id), area) %>%
      group_by_at(12) %>% # council district id index
      mutate(weight = area / sum(area)) %>% 
      # Calculate area-weighted value by GEOID
      summarize(sw_asian_p = sum(weight * asian_p),
                sw_black_p = sum(weight * black_p), 
                sw_hispanic_p = sum(weight * hispanic_p),
                sw_white_p = sum(weight * white_p), 
                sw_total_p = sum(weight * total_p), 
                sw_g20predbid = sum(weight * g20predbid),
                sw_g20prertru = sum(weight * g20prertru))
