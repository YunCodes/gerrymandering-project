...
houston_county_tract <- get_decennial(state = "TX",
                      county = c("201", "157", "339"),
                      geography = "tract",
                      year = 2010,
                      variables = c("black_p" = "P003003",
                                    "white_p" = "P003002",
...
# Create block-level intersenction
for (i in 4) {
    
    # Import city_block_precinct_council_district intersection exported from QGIS
    block_precinct_council_district_intersection <- st_read(paste0(wd, 
                                                                   "CleanedData/QGISOutput/Block_Precinct_CouncilDistrict/", # path to folder 
                                                                   str_replace_all(tolower(cities[i]), " ", "_"), "_block_precinct_council_district_intersection.shp")) 
    
    precinct_id <- cities_precinct_council_district_id_columns[[i]][1]
    city_block_wide <- mget(ls(pattern = paste0("^", str_replace_all(tolower(cities[i]), " ", "_"),
                                                "_city_block$")))[[1]] %>%
      spread(variable, value)
      
    
    intersection_weighted_block <- block_precinct_council_district_intersection %>%
      as_tibble() %>% # Remove geometry for faster computation
      dplyr::select(GEOID, all_of(demographic_columns), 
                    all_of(precinct_id), all_of(partisan_columns),
                    area) %>% # Select relevant columns only 
      group_by(GEOID) %>% # block-level aggregation
      mutate(weight = area / sum(area)) %>% 
      # Calculate area-weighted value by GEOID
      summarize(sw_g20predbid = sum(weight * g20predbid, na.rm = TRUE),
                sw_g20prertru = sum(weight * g20prertru, na.rm = TRUE)) %>% 
      # Attach back block-level demographic data with geography
      left_join(city_block_wide, by = "GEOID")
    
    # Standardize column names 
    intersection_weighted_block <- intersection_weighted_block %>%
      dplyr::select(GEOID, NAME, asian_p, black_p, hispanic_p, white_p, total_p, sw_g20predbid, sw_g20prertru, geometry)
    
    # Save new objects in the environment
    assign(paste0(str_replace_all(tolower(cities[i]), " ", "_"), "_intersection_weighted_block"), 
           intersection_weighted_block)
    
    # Export new objects
    st_write(intersection_weighted_block, 
             paste0(wd, "CleanedData/CityIntersection/Block/", str_replace_all(tolower(cities[i]), " ", "_"), "_intersection_weighted_block.shp"),
             append = FALSE)
}

# Check which blocks have na values
chicago_intersection_weighted_block %>% 
  filter(is.na(sw_g20predbid) & is.na(sw_g20prertru)) %>%
  pull(GEOID)

# Create council district-level intersection
for (i in 4) {
  # Import city_block_precinct_council_district intersection exported from QGIS
  block_council_district_intersection <- st_read(paste0(wd, 
                                                        "CleanedData/QGISOutput/Block_CouncilDistrict/", # path to folder 
                                                        str_replace_all(tolower(cities[i]), " ", "_"), "_block_council_district_intersection.shp")) 
  
  council_district_id <- cities_precinct_council_district_id_columns[[i]][2]
  
  intersection_weighted_council_district <- block_council_district_intersection %>%
    as_tibble() %>%
    dplyr::select(GEOID, all_of(demographic_columns_abrv), 
                  all_of(partisan_columns_abrv), 
                  all_of(council_district_id), area) %>%
    group_by_at(11) %>% # council district id index
    mutate(weight = area / sum(area)) %>% 
    # Calculate area-weighted value by GEOID
    summarize(sw_asian_p = sum(weight * asian_p, na.rm = TRUE),
              sw_black_p = sum(weight * black_p, na.rm = TRUE), 
              sw_hispanic_p = sum(weight * hspnc_p, na.rm = TRUE),
              sw_white_p = sum(weight * white_p, na.rm = TRUE), 
              sw_total_p = sum(weight * total_p, na.rm = TRUE),
              sw_g20predbid = sum(weight * sw_g20prd, na.rm = TRUE),
              sw_g20prertru = sum(weight * sw_g20prr, na.rm = TRUE))
  
  names(intersection_weighted_council_district)[1] <- "council_district_id"
  
  assign(paste0(str_replace_all(tolower(cities[i]), " ", "_"), "_intersection_weighted_council_district"), 
         intersection_weighted_council_district)
  
  write_csv(intersection_weighted_council_district,
            paste0(wd, "CleanedData/CityIntersection/CouncilDistrict/", str_replace_all(tolower(cities[i]), " ", "_"), "_intersection_weighted_council_district.csv"))
